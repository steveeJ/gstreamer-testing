...
plugin_init (GstPlugin * plugin)
{
  ...
  if (!gst_element_register (plugin, "unixclientsrc", GST_RANK_NONE,
          GST_TYPE_UNIX_CLIENT_SRC))
    return FALSE;
  if (!gst_element_register (plugin, "unixserversink", GST_RANK_NONE,
          GST_TYPE_UNIX_SERVER_SINK))
    return FALSE;
